...
func GetPaymentUrlByDocsSalesId(docsSalesId int, inviteToken string) string {
	var order models.TbankOrder
	postgres.DB.Where("docs_sales_id = ?", docsSalesId).Where("invite_token = ?", inviteToken).First(&order)
